...
class SFTDataCollatorWith4DAttentionMask(MultiModalDataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        # NEW: Gemma-3 forces token_type_ids to exist during training; text-only inputs are all zeros
        if "token_type_ids" not in features:                                        # NEW
            features["token_type_ids"] = torch.zeros_like(features["input_ids"])    # NEW
