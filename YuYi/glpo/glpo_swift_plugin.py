"""
GLPO for ms-swift 4.0.1
====================================
Advantage 计算公式:
    base_adv_{ij} = (R_ij - mean_K(R_i)) / std_K(R_i)
    bonus_{ij}   = max(0, R_ij - R_i(G))
    glpo_adv_{ij} = base_adv_{ij} + bonus_{ij}

其中:
    R_ij    : 第 i 组第 j 个样本的 individual reward
    R_i(G)  : 第 i 组的 vote reward (组内常数)
    mean_K, std_K: 在第 i 组内 (K 个样本) 计算

环境变量:
    GLPO_M=4                每 prompt 的 group 数
    GLPO_K=8                每 group 的采样数 (= num_generations)
    GLPO_BONUS_SCALE=1.0    bonus 缩放系数 (默认 1.0; 若想让 bonus 影响小一些可设 0.3)
    GLPO_STD_EPS=1e-4       std 下界, 防止除零
    GLPO_DEBUG=1            打印日志
    GLPO_DEBUG_FIRST_STEPS=3 前几个 step 打印完整 sample-level 数据 (默认 3)
...
def _apply_glpo_patch():
    ...
    def _glpo_compute_advantages(self, inputs, rewards_per_func, batch_encoded_inputs):
        ...
        # n_samples 必须能被 K 整除 (每 K 个一组), M 用于推断 num_unique_prompts
        if n_samples % K != 0:
            if debug:
                print(f"[GLPO] WARN: n_samples={n_samples} not divisible by K={K}, fallback")
            return swift_adv if swift_adv is not None else torch.zeros(n_samples)

        num_groups = n_samples // K  # 总组数 (= P*M)
        ...
        # ============ Step 5: 自己计算 base_adv (K=8 组内归一化, 完全可控) ============
        # reshape: (n_samples,) → (num_groups, K)
        ind_grouped = ind_rewards.view(num_groups, K)
        grp_grouped = group_rewards.view(num_groups, K)

        # 组内 mean / std
        group_mean = ind_grouped.mean(dim=1, keepdim=True)              # (num_groups, 1)
        group_std = ind_grouped.std(dim=1, keepdim=True, unbiased=True) # (num_groups, 1), ddof=1
        group_std = group_std.clamp(min=std_eps)

        base_adv_grouped = (ind_grouped - group_mean) / group_std       # (num_groups, K)
        base_adv = base_adv_grouped.reshape(-1)                         # (n_samples,)

        # ============ Step 6: bonus = max(0, R_ij - R_i(G)) ============
        # 取每组的 R(G) (组内常数, 取第一个值即可)
        R_G_per_group = grp_grouped[:, 0:1]                              # (num_groups, 1)
        margin = ind_grouped - R_G_per_group                             # (num_groups, K)
        bonus_grouped = torch.clamp(margin, min=0.0) * bonus_scale       # (num_groups, K)
        bonus = bonus_grouped.reshape(-1)                                # (n_samples,)
        ...
        if debug and (cur_step <= debug_first_n or cur_step % 20 == 1):
            with torch.no_grad():
                ...
                print(f"\n{'='*70}")
                print(f"[GLPO-DEBUG] step={cur_step} | n={n_samples} K={K} M={M} "
                ...
                if swift_adv is not None:
                    ...
                    print(f"swift_adv group[0] mean={swift_grp_mean:+.4f} "
                          ...
                          f"({'✓ 可能是组内归一' if abs(swift_grp_mean) < 0.01 else '✗ 非组内归一'})")
                print(f"{'='*70}\n")
        ...
        with torch.no_grad():
            ...
            # 集体智慧
            ind_mean_per_group = ind_grouped.mean(dim=1)        # (num_groups,)
            ind_max_per_group = ind_grouped.max(dim=1).values
            R_G_per_group_flat = grp_grouped[:, 0]              # (num_groups,)
    ...
    print(f"[GLPO-FIXED] Patched _compute_advantages.")
    print(f"[GLPO-FIXED] M={M}, K={K}, bonus_scale={bonus_scale}")
    print(f"[GLPO-FIXED] base_adv = (R_ij - mean_K)/std_K  (K组内归一化, 自己实现)")
    print(f"[GLPO-FIXED] bonus    = max(0, R_ij - R_G) * scale")
    print(f"[GLPO-FIXED] glpo_adv = base_adv + bonus")
...
if __name__ == "__main__":
    ...
    # 模拟你给的那组 reward
    R = torch.tensor([[0.3, 0.3, 0.8, 1.2, 3.7, 2.16, 0.95, 0.88]])  # (1组, K=8)
    # 假设 R_G = 0.3 (众数对应的 reward, 简化)
    R_G_val = 0.3
    ...
    print()
    print("预期 (与上一条回复手算结果一致):")
    print("base_adv ≈ [-0.8670, -0.8670, -0.4275, -0.0758, +2.1220, +0.7682, "
          "-0.2956, -0.3571]")
    ...
    assert r_grp[0] == r_grp[1] == r_grp[2] == r_grp[3], "Group 1 内应该相同"
    assert r_grp[4] == r_grp[5] == r_grp[6] == r_grp[7], "Group 2 内应该相同"
